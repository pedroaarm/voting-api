    ...
    cbRegistry =
        CircuitBreakerRegistry.of(
            CircuitBreakerConfig.custom()
                ...
                .recordExceptions(ElegibilidadeIndisponivelException.class)
                .build());
    retryRegistry =
        RetryRegistry.of(
            RetryConfig.custom()
                ...
                .retryExceptions(ElegibilidadeIndisponivelException.class)
                .build());
    var props =
        new ElegibilidadeProperties(
            "user-info", "http://localhost:" + wm.port(), Duration.ofSeconds(2));
  ...
  void timeoutResultaEmIndisponivel() {
    wm.stubFor(
        get(urlEqualTo("/users/999"))
            .willReturn(okJson("{\"status\":\"ABLE_TO_VOTE\"}").withFixedDelay(3000)));
