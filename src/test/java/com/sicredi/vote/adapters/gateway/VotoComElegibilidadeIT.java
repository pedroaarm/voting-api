...
class VotoComElegibilidadeIT extends AbstractPostgresIT {
    ...
    private RestClient rest() { return RestClient.create("http://localhost:" + port); }

    private String abrirPautaComSessao() {
        var rc = rest();
        var pauta = rc.post().uri("/api/v1/pautas").contentType(MediaType.APPLICATION_JSON)
            .body("{\"titulo\":\"P\"}").retrieve().toEntity(String.class);
        String id = com.jayway.jsonpath.JsonPath.read(pauta.getBody(), "$.id");
        rc.post().uri("/api/v1/pautas/{id}/sessoes", id).contentType(MediaType.APPLICATION_JSON)
            .body("{}").retrieve().toBodilessEntity();
        return id;
    }

    private HttpStatusCode votar(String pautaId, String assoc, String cpf) {
        return rest().post().uri("/api/v1/pautas/{id}/votos", pautaId).contentType(MediaType.APPLICATION_JSON)
