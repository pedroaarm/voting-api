  ...
  void fluxoCompletoCadastrarAbrirVotarResultado() {
    ...
    var pauta =
        rest.post()
            ...
            .body("{\"titulo\":\"Reforma\",\"descricao\":\"d\"}")
            .retrieve()
    ...
    rest.post()
        ...
        .contentType(MediaType.APPLICATION_JSON)
        .body("{\"associadoId\":\"a1\",\"cpf\":\"1\",\"opcao\":\"SIM\"}")
        .retrieve()
        .toBodilessEntity();
    rest.post()
        .uri("/api/v1/pautas/{id}/votos", pautaId)
        .contentType(MediaType.APPLICATION_JSON)
        .body("{\"associadoId\":\"a2\",\"cpf\":\"2\",\"opcao\":\"SIM\"}")
        .retrieve()
        .toBodilessEntity();
    rest.post()
        .uri("/api/v1/pautas/{id}/votos", pautaId)
        .contentType(MediaType.APPLICATION_JSON)
        .body("{\"associadoId\":\"a3\",\"cpf\":\"3\",\"opcao\":\"NAO\"}")
        .retrieve()
        .toBodilessEntity();
    ...
    var resultado =
        rest.get().uri("/api/v1/pautas/{id}/resultado", pautaId).retrieve().toEntity(String.class);
    assertThat(resultado.getStatusCode()).isEqualTo(HttpStatus.OK);
    assertThat((String) com.jayway.jsonpath.JsonPath.read(resultado.getBody(), "$.status"))
        .isEqualTo("APROVADA");
    assertThat(
            ((Number) com.jayway.jsonpath.JsonPath.read(resultado.getBody(), "$.totalSim"))
                .intValue())
        .isEqualTo(2);
  }
