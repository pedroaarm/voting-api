...
class TelaContratoTest {
    ...
    void telaPautasListaApontaParaEndpointsReais() throws Exception {
        ...
        when(listarPautas.executar()).thenReturn(List.of(
            Pauta.builder().id(pautaId).titulo("Reforma do estatuto").descricao("desc").build()));

        mvc.perform(get("/api/v1/telas/pautas"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.tipo").value("SELECAO"))
            .andExpect(jsonPath("$.itens[0].url").value("http://localhost:8080/api/v1/telas/pautas/" + pautaId));
    }
    ...
    void telaAcoesPautaRetornaSelecaoComEndpointsReais() throws Exception {
        ...
        mvc.perform(get("/api/v1/telas/pautas/{id}", pautaId))
            ...
            .andExpect(jsonPath("$.itens[0].url").value("http://localhost:8080/api/v1/telas/pautas/" + pautaId + "/sessao/nova"))
            .andExpect(jsonPath("$.itens[1].url").value("http://localhost:8080/api/v1/telas/pautas/" + pautaId + "/voto"))
            .andExpect(jsonPath("$.itens[2].url").value("http://localhost:8080/api/v1/telas/pautas/" + pautaId + "/resultado"));
    }
    ...
    void telaVotoOpcoesRetornaSelecaoComPayload() throws Exception {
        ...
        mvc.perform(post("/api/v1/telas/pautas/{id}/voto/opcoes", pauta)
            ...
            .andExpect(jsonPath("$.itens[0].url").value("http://localhost:8080/api/v1/pautas/" + pauta + "/votos"));
    }
