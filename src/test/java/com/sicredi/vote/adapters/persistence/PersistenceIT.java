...
class PersistenceIT extends AbstractPostgresIT {
    ...
    void salvaERecuperaPautaSessaoEVoto() {
        ...
        Pauta pauta = pautas.salvar(Pauta.builder().id(UUID.randomUUID())
            .titulo("Pauta X").descricao("desc").criadaEm(agora).build());
        ...
        votos.salvar(Voto.builder().id(UUID.randomUUID()).pautaId(pauta.getId())
            .associadoId("assoc-1").opcao(OpcaoVoto.SIM).criadoEm(agora).build());
