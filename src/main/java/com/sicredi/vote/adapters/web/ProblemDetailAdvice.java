...
public class ProblemDetailAdvice {
    ...
    private ProblemDetail problem(HttpStatus status, String slug) {
        String title  = messages.getMessage("problem." + slug + ".title", null, LocaleContextHolder.getLocale());
        String detail = messages.getMessage("problem." + slug + ".detail", null, LocaleContextHolder.getLocale());
    ...
    ProblemDetail handle(PautaNaoEncontradaException e) { return problem(HttpStatus.NOT_FOUND, "pauta-nao-encontrada"); }

    @ExceptionHandler(SessaoJaAbertaException.class)
    ProblemDetail handle(SessaoJaAbertaException e) { return problem(HttpStatus.CONFLICT, "sessao-ja-aberta"); }

    @ExceptionHandler(VotoDuplicadoException.class)
    ProblemDetail handle(VotoDuplicadoException e) { return problem(HttpStatus.CONFLICT, "voto-duplicado"); }

    @ExceptionHandler(SessaoFechadaException.class)
    ProblemDetail handle(SessaoFechadaException e) { return problem(HttpStatus.UNPROCESSABLE_ENTITY, "sessao-fechada"); }

    @ExceptionHandler(SessaoEmAndamentoException.class)
    ProblemDetail handle(SessaoEmAndamentoException e) { return problem(HttpStatus.CONFLICT, "sessao-em-andamento"); }
    ...
    ProblemDetail handleUnreadable(org.springframework.http.converter.HttpMessageNotReadableException e) {
        return problem(org.springframework.http.HttpStatus.BAD_REQUEST, "validacao");
    }

    @ExceptionHandler(org.springframework.web.method.annotation.MethodArgumentTypeMismatchException.class)
    ProblemDetail handleTypeMismatch(org.springframework.web.method.annotation.MethodArgumentTypeMismatchException e) {
        return problem(org.springframework.http.HttpStatus.BAD_REQUEST, "validacao");
    }
}
