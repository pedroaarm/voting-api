...
public final class SessaoMapper {
    ...
    public static SessaoJpaEntity toEntity(Sessao s) {
        return new SessaoJpaEntity(s.getId(), s.getPautaId(), s.getAbertura(), s.getFechamento(), s.getAbertura());
    }
    public static Sessao toDomain(SessaoJpaEntity e) {
        return Sessao.builder().id(e.getId()).pautaId(e.getPautaId())
            .abertura(e.getAbertura()).fechamento(e.getFechamento()).build();
    }
}
