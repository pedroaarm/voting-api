...
public class PautaRepositoryAdapter implements PautaRepository {
    ...
    public PautaRepositoryAdapter(PautaJpaRepository jpa) { this.jpa = jpa; }

    @Override public Pauta salvar(Pauta pauta) {
        return PautaMapper.toDomain(jpa.save(PautaMapper.toEntity(pauta)));
    }
    @Override public Optional<Pauta> buscarPorId(UUID id) {
    ...
    @Override public java.util.List<com.sicredi.vote.domain.model.Pauta> listarTodas() {
        return jpa.findAll().stream().map(com.sicredi.vote.adapters.persistence.mapper.PautaMapper::toDomain).toList();
    }
}
